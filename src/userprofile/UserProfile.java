...
/**
 * Object to store persistent data about the current user and distribute the data to the other modules.
 * Includes methods for accessing configurations and user credentials.
 * 
 * @author xv435
 *
 */
public class UserProfile {
	...
	/**
	 * This method updates the User Profile Configurations map so that is is in sync with the config object.
	 */
	private void updateMap() {
		configMap = new HashMap<String, Serializable>();
		
		//These four are the only configurations that need to have getter/setters as of now
		//TODO Add getters from config to the map rather than null
		configMap.put("rootfolder", null);
		configMap.put("port", null);
		configMap.put("maxfilesize", null);
		configMap.put("iplist", null);
	}
	
	/**
	 * This constructor creates a User Profile from a username, password, and configurations object
	 * 
	 * @param username The user's username
	 * @param password The user's password
	 * @param config A configuration object to use
	 */
	...
	/**
	 * This constructor is only for use with non-default PINs
	 */
	public UserProfile(String username, String password, String pin, PersystConfiguration config) {
		this.username = username;
		this.password = password;
		this.pin = pin;
		this.config = config;
	 ...
	 * 
	 * @param configuration The name of the configuration to set
	 * @param value A serializable object to set
	 */
	 ...
	 * 
	 * @param config Object to set configurations to
	 */
